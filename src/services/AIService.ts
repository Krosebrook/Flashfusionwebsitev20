...
Format as JSON:
{
  ...
  // ========================================================================
  // Additional AI Methods for Tool Execution Service
  // ========================================================================
  ...
  public async generateImage(params: {
    ...
    // In demo mode or without API keys, return mock data
    const hasOpenAIKey = await this.hasValidApiKey('openai');
    ...
    try {
      ...
      return {
        imageUrl: 'https://picsum.photos/512/512', // Placeholder
        prompt: params.prompt,
        dimensions: { width: 512, height: 512 },
        format: 'png',
        model: params.model || 'dall-e-3',
        note: 'Real image generation requires OpenAI API integration',
      };
    } catch (error) {
  ...
  public async analyze(params: {
    ...
    const systemPrompt = `You are an expert code and content analyzer. Analyze the provided content and return detailed insights in JSON format.`;

    const userPrompt = `Analyze this ${params.type}:
...
Return as JSON.`;
  ...
  public async complete(params: {
    ...
    const response = await this.makeAIRequest({
      prompt: params.prompt,
      model: params.model || this.selectedModel,
      provider: params.provider || this.selectedProvider,
