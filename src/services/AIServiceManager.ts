...
export class AIServiceManager {
  ...
  constructor() {
    ...
    // Auto-enable demo mode if no providers are configured
    if (this.providers.size === 0) {
      console.log('No AI providers configured. Enabling Demo Mode.');
  ...
  private generateMockResponse(request: AIRequest): Promise<AIResponse> {
    return new Promise((resolve) => {
      setTimeout(() => {
        let content = '';
        if (request.systemPrompt?.includes('software developer')) {
          content = this.getMockCodeResponse(request.prompt);
        } else if (request.systemPrompt?.includes('content creator')) {
          content = this.getMockContentResponse(request.prompt);
        } else if (request.systemPrompt?.includes('analyze the provided code')) {
           content = this.getMockAnalysisResponse(request.prompt);
        } else {
          content = `[DEMO MODE] Generated response for: ${request.prompt}\n\nThis is a simulated AI response because the application is running in Demo Mode. To use real AI generation, please configure valid API keys.`;
        }
  ...
  private getMockCodeResponse(prompt: string): string {
    return `// [DEMO] Generated code for: ${prompt}
import React, { useState, useEffect } from 'react';

/**
 * Demo Component
 * This is a simulated component generated in Demo Mode.
 */
  ...
  return (
    <div className="p-4 border rounded-lg shadow-sm">
      ...
      <button 
        ...
        onClick={() => alert('Interaction working!')}
      >
        Click Me
      </button>
    </div>
...
export default DemoComponent;`;
  ...
  private getMockContentResponse(prompt: string): string {
    return `# [DEMO] Content: ${prompt}

## Introduction
This is a demonstration of the content generation capabilities. In a live environment, this would be generated by a powerful LLM like GPT-4 or Claude 3.5 Sonnet.

## Key Features
- **Instant Generation**: Creates content in seconds.
- **Context Aware**: Understands your topic and audience.
- **SEO Optimized**: Includes relevant keywords automatically.

## Conclusion
To unlock the full potential of FlashFusion, please configure your AI provider API keys in the settings.

#AI #Demo #FlashFusion`;
  }
  
  private getMockAnalysisResponse(prompt: string): string {
    return `Analysis Report [DEMO]
    
1. **Code Quality**: The code structure appears sound, but this is a simulated analysis.
2. **Performance**: Consider using memoization for expensive calculations.
3. **Security**: Ensure all user inputs are sanitized.
4. **Recommendation**: Refactor the main loop for better readability.

*Note: Real analysis requires an active AI provider connection.*`;
  }
...
- Include necessary imports and dependencies`;
    ...
    return this.generateContent({
      prompt,
      systemPrompt, // This helps identify the request type in mock mode
      temperature: 0.2,
      maxTokens: 6000
    });
  }
